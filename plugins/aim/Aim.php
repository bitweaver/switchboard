<?php
/**
 * @version $Header: /cvsroot/bitweaver/_bit_switchboard/plugins/aim/Aim.php,v 1.2 2009/03/31 05:53:55 lsces Exp $
...
class Aim
{
	...
	/**
	 * The file Resource ID for the log file
	 * 
	 * @var Resource
	 * @access private
	 */
	 ...
	 * @param Boolean $silent Output Debug Statements
	 * @access public
	 */
	function Aim($sn, $password, $pdmode, $silent = false)
    {
		...
		$this->logHandle=fopen("./PHPTocLibdbgout.txt","a+");
		fwrite($this->logHandle,"\r\n New session started at:" . date("M d Y, h:i:s a") . str_repeat("-",100) .  "\r\n\r\n");
		$this->myLogging=true;
		...
		sEcho("PHPTOCLIB v" . VERSION . " Object Created");
		
	}

	/** 
	 * Enables debug logging (Logging is enabled by default)
	 *
	 * 
	 * @access public
	 * @return void
	...
	{
		$this->myLogging=false;
	}
	
	 /** 
	 * Logs a packet to $this->logHandle
	 *
	 * 
	 * @access private
	...
	function setBuddyList()
	{
		...
		{
			sEcho("setBuddyList cannot be called at this time because I got $message");
			return false;
		}
		$people = explode("\n",trim($message,"\n"));
	...
	function sendDim($to, $message)
	{
		...
		if (!$dcon)
		{
			sEcho("Could not find a direct connection to $to");
			return false;
		}
	...
	{
		sEcho("onConfig Message: " . $data);
		
		if (strpos($data,"CONFIG2:") === false)
		{
			sEcho("get_buddy_list cannot be called at this time because I got $data");
			//return false;
		}
		$people = explode("\n",trim($data,"\n"));
		//The first 3 elements of the array are who knows what, element 3 should be
	...
	{
		global $ERRORS;
		@list($cmd, $errornum, $errargs) = explode(":", $data);
...
{
	return msg_parse(msg_type($message));
}

/** 
 * Silect/Debug Echo
 *
 * Only echos if the Silent option is disabled
 *
 * @access public
 * @param String $out String to be echoed
 * @param boolean $newline Appends new line if true (default true)
 * @return void
 */
